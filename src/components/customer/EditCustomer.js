...
const EditCustomer = (props) => {
  ...
  useEffect(() => {
    ...
    const companyNumber = customer.phones.find(
      (p) => p.alias === Phone.COMPANY
    );
    if (companyNumber) {
      setWorkNumber(companyNumber.phone);
    }

    const homeAddress = customer.addresses.find(
      (a) => a.alias === Address.HOME
    );
    if (homeAddress) {
    ...
    const companyAddress = customer.addresses.find(
      (a) => a.alias === Address.COMPANY
    );
    if (companyAddress) {
  ...
  const saveHandler = () => {
    ...
    if (homeCity !== "" && homeDistrict !== "" && homeRoad !== "") {
      const oldRecord = customer.addresses.find(
        (p) => p.alias === Address.HOME
      );
      if (oldRecord) {
  ...
  return (
    <form>
      ...
      <div className="flex flex-row-reverse mt-8">
        <Button color="success" disabled={props.isSaving} onClick={saveHandler}>
